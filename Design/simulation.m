...
tfDY = feedback(P,F*K2); tfDU = feedback(P*F*K2,1);
tfNY = feedback(F,K2*P); tfNU = feedback(F*K2,P);
...
figure; 
subplot 211; plot(t,y1); title('y with K1 (Dmin=1)');
subplot 212; plot(t,y2); title('y with K2 (Dmin=0.5)');
figure; 
subplot 211; plot(t,u1); title('u with K1 (Dmin=1)');
subplot 212; plot(t,u2); title('u with K2 (Dmin=0.5)');